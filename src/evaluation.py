...
class ModelEvaluator:
    ...
    def __init__(self):
        ...
        if SENTENCE_TRANSFORMERS_AVAILABLE:
            ...
            self.use_sentence_transformers = False
            print("Warning: sentence-transformers not available, using TF-IDF for cosine similarity")
    
    def evaluate_accuracy_cosine(self, model_output: str, reference_answer: str) -> float:
        """
        ...
        """
        if not model_output.strip() or not reference_answer.strip():
            return 0.0
            
        try:
            if self.use_sentence_transformers:
                # Use sentence transformers for better semantic similarity
                embeddings = self.sentence_model.encode([model_output, reference_answer])
                similarity = cosine_similarity([embeddings[0]], [embeddings[1]])[0][0]
            else:
                # Fallback to TF-IDF based cosine similarity
                vectorizer = TfidfVectorizer(stop_words='english', lowercase=True)
                tfidf_matrix = vectorizer.fit_transform([model_output, reference_answer])
            ...
            print(f"Warning: Error computing cosine similarity: {e}")
            return 0.0
    ...
    def evaluate_response(
        ...
        Args:
            ...
            reference_answer: Optional reference answer for cosine similarity
        ...
        # Add cosine similarity accuracy if reference answer is provided
        if reference_answer:
            results["accuracy_cosine"] = self.evaluate_accuracy_cosine(response, reference_answer)
        
        # Add confidence metrics if available
        if confidence_metrics:
            confidence_eval = self.evaluate_confidence(confidence_metrics)
            results.update(confidence_eval)
